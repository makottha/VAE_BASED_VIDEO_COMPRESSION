"""
preprocess_128.py — UCF101 preprocessing for 128×128 MS-VQ-VAE-3 training

Reads raw UCF101 .avi files, extracts 32 frames per clip at 16 FPS,
resizes to 128×128, and saves as float32 .pt tensors.

Output structure mirrors dataset_64/:
    dataset_128/
        train/tensors/*.pt
        val/tensors/*.pt
        test/tensors/*.pt

Each .pt file: tensor of shape (3, 32, 128, 128), float32, values in [0, 1]

Features:
  - Resumable: skips clips already saved
  - Progress bar per video (tqdm)
  - Same train/val/test split ratios as 64×64 pipeline (0.70 / 0.15 / 0.15)
  - Estimated output size: ~60-80 GB

Usage:
    conda activate vae_env
    cd C:/Users/kmani/PycharmProjects/VAE_BASED_VIDEO_COMPRESSION/MS_VQ_VAE_128
...
OUTPUT_DIR      = "../dataset_128"          # where tensors will be saved
TARGET_FPS      = 16
...
def process_video(video_path: str) -> int:
    ...
    while True:
        ...
        if frame_count % frame_interval == 0:
            ...
            frames.append(transform(frame_rgb))  # (3, 128, 128)

            if len(frames) == CLIP_FRAMES:
                out_path = get_tensor_path(video_name, clip_idx, split)

                if not os.path.exists(out_path):
                    clip_tensor = torch.stack(frames, dim=1)  # (3, 32, 128, 128)
