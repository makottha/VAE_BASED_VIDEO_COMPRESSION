"""
eval_h265_128.py — H.265/HEVC baseline evaluation at 128x128
...
CRF range [28-51] matches the H.264 sweep.  H.265 achieves ~50% lower
...
Outputs saved to outputs_msvqvae_128/h265_baseline/:
...
OUT_DIR  = "./outputs_msvqvae_128/h265_baseline"
N_CLIPS  = 500
CRFS     = [28, 32, 36, 40, 44, 48]   # same range as H.264 for direct comparison
WIDTH    = 128
HEIGHT   = 128
T_FRAMES = 32
FPS      = 16
...
# ---- I/O helpers (identical to eval_h264_128.py) ----------------------------
...
with open(per_clip_csv, "w", newline="") as f, open(log_path, "a") as log_f:
    ...
    log_f.write(f"\n{'='*60}\n")
    log_f.write(f"H.265/HEVC Baseline 128x128 -- {run_ts}\n")
