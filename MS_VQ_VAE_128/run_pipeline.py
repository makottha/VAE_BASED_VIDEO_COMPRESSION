"""
run_pipeline.py — Master orchestration script for 128×128 MS-VQ-VAE-3 pipeline

Execution order:
  Step 1: Preprocess UCF-101 → dataset_128/  (skips already-done clips)
...
Resumable: state tracked in outputs_msvqvae_128/pipeline_status.json
Re-run this script at any time to continue from where it left off.

All logs written to outputs_msvqvae_128/:
...
def step_preprocess(status: dict):
    ...
    ok = run_step(
        ...
        "Step 1: Preprocess UCF-101 → dataset_128",
    )
    if ok:
...
def step_train_k(K: int, status: dict):
    ...
    # Patch train_128.py to run only this K value
    # We pass K as an environment variable and read it in a wrapper
    env_patch = os.environ.copy()
    env_patch["PIPELINE_K"] = str(K)

    ok = run_step(
        [PYTHON, "train_single_k.py", str(K)],
...
def main():
    log("="*60)
    log("MS-VQ-VAE-3  128×128  Pipeline")
    log("="*60)
