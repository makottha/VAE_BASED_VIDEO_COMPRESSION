"""
train_128.py — Three-level MS-VQ-VAE training at 128×128 resolution
...
Hardware config (verified safe on RTX 4060 8GB):
  BATCH_SIZE = 2, GRAD_ACCUM_STEPS = 4  →  effective batch = 8

Checkpoints saved to: outputs_msvqvae_128/
  ae_K{K}_epoch{N}.pt
...
# Dataset (128×128 preprocessed tensors)
...
# Data
EXPECTED_T      = 32
SUBSET_FRACTION = 0.5   # use 50% of clips per run (consistent with 64×64 protocol)

# Hardware (safe config for RTX 4060 8GB at 128×128)
BATCH_SIZE       = 2
...
# K sweep — same values as 64×64 for fair comparison
...
def run_stage_a(K: int, device: torch.device):
    print(f"\n{'='*60}")
    print(f"  STAGE A — Autoencoder  |  K={K}  |  128×128")
    print(f"{'='*60}")
    ...
    for epoch in range(1, EPOCHS_STAGE_A + 1):
        ...
        save_ckpt(ae, opt, epoch, ckpt_path)
...
def run_stage_b(K: int, ae: MSVQVAE3Video, device: torch.device):
    print(f"\n{'='*60}")
    print(f"  STAGE B — Priors  |  K={K}  |  128×128")
    print(f"{'='*60}")
